...
export default function MyOrders() {
  ...
  const pendingAmount = orders.filter(o => o.payment_status === 'pending' && o.status !== 'cancelled')
  ...
  return (
    <AppLayout>
      ...
      <div className="metrics-grid">
        {[
          { icon: '📦', label: 'Total Orders',    value: totalOrders,                              color: 'var(--ember)',  bg: 'var(--ember-glow)' },
          { icon: '🛍️', label: 'Total Items',     value: totalItems,                               color: 'var(--blue)',   bg: 'var(--blue-dim)'   },
          { icon: '₹',  label: 'Total Spent',     value: `₹${totalSpent.toLocaleString('en-IN')}`, color: 'var(--green)',  bg: 'var(--green-dim)'  },
          { icon: '⏳', label: 'Pending Payment', value: `₹${pendingAmount.toLocaleString('en-IN')}`, color: 'var(--ember)', bg: 'var(--ember-glow)' },
        ].map((m, i) => (
      ...
      <div className="panel">
        ...
        {loading ? (
          <div style={{ padding: 30, color: 'var(--text-muted)' }}>Loading orders...</div>
        ) : filtered.length === 0 ? (
          ...
          <table className="data-table">
            ...
            <tbody>
              {filtered.map((o, index) => {
                ...
                const payment = PAYMENT_BADGE[o.payment_status] || { label: o.payment_status, color: '#888', bg: '#88888820' }

                return (
                  <tr key={o._id} style={{ opacity: o.status === 'cancelled' ? 0.6 : 1 }}>
                    ...
                    {/* Date & Time */}
                    <td>
                      <div style={{ fontWeight: 600, fontSize: 13 }}>{formatDate(o.createdAt)}</div>
                      <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{formatTime(o.createdAt)}</div>
                    </td>

                    {/* Items */}
                    <td>
                      {o.items?.map((item, i) => (
                        <div key={item._id || i} style={{ fontSize: 13, marginBottom: 2 }}>
                          <span style={{ fontWeight: 600 }}>{item.name}</span>
                          <span style={{ color: 'var(--text-muted)', fontSize: 12 }}>
                            {' '}× {item.quantity}
                            {' '}· <span style={{ fontFamily: 'var(--font-mono)' }}>₹{item.total}</span>
                          </span>
                        </div>
                      ))}
                    </td>
                    ...
                    {/* Amount */}
                    <td className="td-mono" style={{ fontWeight: 700, fontSize: 14 }}>
