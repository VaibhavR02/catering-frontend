...
export default function MasterDashboard() {
  ...
  // Society-wise tiffin count
  const societyMap = {}
  orders.forEach(o => {
    if (!societyMap[o.society]) societyMap[o.society] = { name: o.society.replace(' Society',''), tiffins: 0, orders: 0 }
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div style={{ background: '#1A2130', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: '10px 14px', fontSize: 12 }}>
  ...
  return (
    <AppLayout pendingCount={pending}>
      ...
      <div className="metrics-grid">
        {[
          { icon: '📦', label: 'Total Orders',  value: totalOrders,   sub: `${pending} pending`,   color: 'var(--ember)',  bg: 'var(--ember-glow)',  deltaClass: pending > 0 ? 'delta-down' : 'delta-up' },
          { icon: '🍱', label: 'Total Tiffins', value: totalTiffins,  sub: 'today',                color: 'var(--blue)',   bg: 'var(--blue-dim)',    deltaClass: 'delta-up' },
          { icon: '🚀', label: 'Delivery Rate', value: `${deliveryRate}%`, sub: `${delivered} delivered`, color: 'var(--green)', bg: 'var(--green-dim)', deltaClass: 'delta-up' },
          { icon: '₹',  label: 'Collected',     value: `₹${paidAmount}`, sub: `₹${totalAmount - paidAmount} pending`, color: 'var(--purple)', bg: 'var(--purple-dim)', deltaClass: paidAmount < totalAmount ? 'delta-down' : 'delta-up' },
        ].map((m, i) => (
          <div className="metric-card" key={m.label} style={{ animationDelay: `${i*0.07}s` }}>
            ...
            <div className={`metric-delta ${m.deltaClass}`}>
              {m.deltaClass === 'delta-up' ? '↑' : '↓'} <span className="delta-note">{m.sub}</span>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="panel">
          <div className="panel-header">
            <div>
              ...
              <div className="panel-subtitle">Orders & deliveries this week</div>
            </div>
          </div>
        ...
        <div className="panel">
          ...
          <div className="panel-body" style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <ResponsiveContainer width={130} height={130}>
              <PieChart>
                <Pie data={statusPie} cx="50%" cy="50%" innerRadius={38} outerRadius={60} paddingAngle={3} dataKey="value">
                  {statusPie.map((_, i) => <Cell key={i} fill={['#10B981','#3B82F6','#F59E0B'][i]} />)}
                </Pie>
              </PieChart>
            </ResponsiveContainer>
            <div style={{ flex: 1 }}>
              {statusPie.map((s, i) => (
                <div key={s.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--text-primary)' }}>{s.value}</span>
                </div>
              ))}
              <div className="divider" />
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Delivery Rate</span>
                <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--green)', fontFamily: 'var(--font-mono)' }}>{deliveryRate}%</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="panel">
        <div className="panel-header">
          <div>
            ...
            <div className="panel-subtitle">All orders today · auto-refreshing</div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <span className="chip"><span className="live-dot" style={{ marginRight: 4 }} />Live</span>
          </div>
        </div>
            ...
            <tbody>
              {orders.map(o => (
                <tr key={o.id}>
                  ...
                  <td><span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, color: 'var(--ember)' }}>{o.count}</span></td>
                  ...
                  <td className="td-mono" style={{ color: o.payStatus === 'paid' ? 'var(--green)' : 'var(--text-secondary)' }}>
                    ₹{o.amount}
                  </td>
                </tr>
              ))}
